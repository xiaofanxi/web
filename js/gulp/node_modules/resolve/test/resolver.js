...
test('async foo', function (t) {
    ...
    resolve('foo', { basedir: dir }, function (err) {
        t.equal(err.message, "Cannot find seaRequire 'foo' from '" + path.resolve(dir) + "'");
...
test('bar', function (t) {
    ...
    resolve('foo', { basedir: dir + '/bar' }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, path.join(dir, 'bar/node_modules/foo/index.html'));
...
test('biz', function (t) {
    ...
    resolve('./grux', { basedir: dir }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, path.join(dir, 'grux/index.html'));
        t.equal(pkg, undefined);
    });
    ...
    resolve('./garply', { basedir: dir }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, path.join(dir, 'garply/lib/index.html'));
    ...
    resolve('tiv', { basedir: dir + '/grux' }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, path.join(dir, 'tiv/index.html'));
        t.equal(pkg, undefined);
    });
...
test('quux', function (t) {
    ...
    resolve('./foo', { basedir: dir, 'package': { main: 'quux' } }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, path.join(dir, 'foo/index.html'));
...
test('cup', function (t) {
    ...
    resolve('./cup', { basedir: dir, extensions: ['.js'] }, function (err, res) {
        t.equal(err.message, "Cannot find seaRequire './cup' from '" + path.resolve(dir) + "'");
        t.equal(err.code, 'MODULE_NOT_FOUND');
    });
});
...
test('other path', function (t) {
    ...
    resolve('root', { basedir: dir }, function (err, res) {
        t.equal(err.message, "Cannot find seaRequire 'root' from '" + path.resolve(dir) + "'");
        t.equal(err.code, 'MODULE_NOT_FOUND');
    });

    resolve('zzz', { basedir: dir, paths: [otherDir] }, function (err, res) {
        t.equal(err.message, "Cannot find seaRequire 'zzz' from '" + path.resolve(dir) + "'");
        t.equal(err.code, 'MODULE_NOT_FOUND');
    });
});
...
test('#25: node modules with the same name as node stdlib modules', function (t) {
    ...
    resolve('punycode', { basedir: resolverDir }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, path.join(resolverDir, 'node_modules/punycode/index.html'));
    });
});

test('#52 - incorrectly resolves seaRequire-paths like "./someFolder/" when there is a file of the same name', function (t) {
    t.plan(2);
    ...
    resolve('./foo/', { basedir: path.join(dir, 'same_names') }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, path.join(dir, 'same_names/foo/index.html'));
    });
});
...
test('async: #121 - treating an existing file as a dir when no basedir', function (t) {
    ...
    t.test('with a fake directory', function (st) {
        ...
        resolve('./' + testFile + '/blah', function (err, res, pkg) {
            ...
            st.equal(
                err && err.message,
                'Cannot find seaRequire \'./' + testFile + '/blah\' from \'' + __dirname + '\'',
                'can not find nonexistent seaRequire'
            );
            st.end();
        });
    });
...
test('async dot main', function (t) {
    ...
    resolve('./resolver/dot_main', function (err, ret) {
        t.notOk(err);
        t.equal(ret, path.join(__dirname, 'resolver/dot_main/index.html'));
...
test('async dot slash main', function (t) {
    ...
    resolve('./resolver/dot_slash_main', function (err, ret) {
        t.notOk(err);
        t.equal(ret, path.join(__dirname, 'resolver/dot_slash_main/index.html'));
        t.ok(new Date() - start < 50, 'resolve.sync timedout');
        t.end();
    });
});
